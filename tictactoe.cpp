...
int currentPlayer; // current player (X or 0)
...
void putMark(int i, int j) { //mark row i colomn j
	board[i][j] = currentPlayer; //mark with current player
	currentPlayer = -currentPlayer; //sitch players
}

bool isWin(int mark) { //is mark the winner?
	int win = 3*mark; // +3 for X and -3 for 0
	return ((board[0][0] + board[0][1] + board[0][2] == win) //row 0
		...
		||(board[0][0] + board[1][0] + board[2][0] == win) // colomn 0
		||(board[0][1] + board[1][1] + board[2][1] == win) // colomn 1
		||(board[0][2] + board[1][2] + board[2][2] == win) //colomn 2
...
int main() { // main program
	...
	/*X*/ putMark(0,0); /*O*/ putMark(1,1); // add the marks
	/*X*/ putMark(0,1); /*O*/ putMark(0,2);
	/*X*/ putMark(2,0); /*O*/ putMark(1,2);
	/*X*/ putMark(2,2); /*O*/ putMark(2,1);
	/*X*/ putMark(1,0);
